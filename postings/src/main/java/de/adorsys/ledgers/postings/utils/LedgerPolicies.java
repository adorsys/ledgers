...
public class LedgerPolicies {
	...
	public void validatePostingTime(Posting posting){
		if(posting.getPstTime()!=null && posting.getPstTime().isAfter(ledger.getLastClosing())) return;
	...
	public void validateProperAccount(LedgerAccount ledgerAccount) {
		if(ledgerAccount.getLedger()!=null && ledgerAccount.getLedger().getId().equals(ledger.getId())) return;
