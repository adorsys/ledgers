...
public class DepositAccountConfiguration {
	...
	@Autowired
	private ASPSPConfigSource configSource;
	
	
	@Bean
	public ASPSPConfigData initConfigData() throws JsonParseException, JsonMappingException, IOException {
	...
	private LedgerAccountBO newLedgerAccount(LedgerBO ledger, LedgerAccountModel model)
		...
		Optional<LedgerAccountBO> accOption = ledgerService.findLedgerAccount(ledger, model.getName());
		if(accOption.isPresent()) {
			return accOption.get();
		}
		
		LedgerAccountBO parent = null;
		if(model.getParent()!=null) {
			parent = ledgerService.findLedgerAccount(ledger, model.getParent()).orElseThrow(() -> new IllegalStateException(String.format("Missing ledger account with id %s", model.getParent())));
		}
