...
public class MiddlewareServiceImplIT {
    ...
    @Autowired
    private MiddlewareOnlineBankingService onlineBankingService;
    @Before
    ...
    @Test
    public void test(){}
   /* @Test     //TODO fix this AuthToken problem -> "anonymous" not the requested user
    public void execute_payment_read_tx_ok()
            throws AccountNotFoundMiddlewareException, PaymentProcessingMiddlewareException,
                           PaymentNotFoundMiddlewareException, UserAlreadyExistsMiddlewareException,
                           UserNotFoundMiddlewareException, InsufficientPermissionMiddlewareException,
        ...
        String authorise = onlineBankingService.authorise(users.get(0).getLogin(), users.get(0).getPin(), UserRoleTO.CUSTOMER);
    ...
    private void checkBalances(List<AccountBalances> list, LedgerBO ledgerBO) {
        if (list == null) {
            return;
        }
