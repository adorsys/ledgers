...
public class MiddlewareOauthServiceImpl implements MiddlewareOauthService {
    ...
    public OauthCodeResponseTO oauthCode(String login, String pin, String redirectUri) {
        ...
        OauthCodeResponseBO response = oauthAuthorisationService.oauthCode(userId, token.getAccess_token());
    ...
    public OauthCodeResponseTO oauthCode(ScaInfoTO scaInfo, String redirectUri) {
        OauthCodeResponseBO response = oauthAuthorisationService.oauthCode(scaInfo.getUserId(), scaInfo.getAccessToken());
        return new OauthCodeResponseTO(redirectUri, response.getCode());
    }

    @Override
    public BearerTokenTO oauthToken(String code) {
        String oldToken = oauthAuthorisationService.oauthToken(code);
        return tokenService.exchangeToken(oldToken, fullTokenLifeTime, SCOPE_FULL_ACCESS);
    }
